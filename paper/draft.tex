...
\title{Highly efficient implementation of penalized models and their application to genetic risk prediction}
...
\abstract{
Polygenic Risk Scores (PRSs) consist in combining the information across many single-nucleotide polymorphisms (SNPs) in a score reflecting the genetic risk of developing a disease. 
PRSs might have a major public health impact, possibly allowing for screening campaigns to identify high-genetic risk individuals for a given disease.  The most commonly used method to derive PRSs is called ``Clumping+Thresholding'' (C+T). It uses only univariate genome-wide association studies (GWAS) summary statistics, which makes it very suitable and fast. However, these PRS rely on non-optimal heuristics. 

In R package bigstatsr, we implemented linear and logistic regression models that can be used to jointly estimate SNP effects for computing PRS. We use an efficient algorithm with a new implementation in order for this model to be used on huge datasets.

In this paper, we present a comprehensive comparative study of the C+T method, our penalized logistic regression and a derivation of random forests. The penalized logistic regression consistently achieves higher predictive performance than the two other methods while being faster than the C+T method. 
...
The improvement in predictive performance is more pronounced when there are few effects located in nearby genomic regions with correlated SNPs, and with an increasing sample size. 
When analyzing real SNP data from a case-control study for celiac disease, penalized logistic regression provides an AUC of 89\% while the standard C+T method provides a smaller AUC of 82\%.

[TOMOD] Instead of computing polygenic risk scores based on GWAS univariate summary statistics, we recommend to use penalized logistic regression as implemented in the R package bigstatsr to obtain more accurate polygenic risk scores when individual genotype data are available.\\
...
Yet, models resulting from LMM, known e.g.\ as ``gBLUP'', are not optimal for predicting disease status based on genotypes \cite[]{abraham2013performance}. 
...
For the penalized logistic regression, we include two practical improvements. 
...
Secondly, we also show how to use feature engineering in order to capture not only linear effects, but also recessive and dominant effects.
...
[TOMOD] In this study, we particularly extend the work of \cite{abraham2013performance} by using a simulation framework similar but more extenstive than the one used in \cite{zhou2013polygenic}. 
...
[TOMOD] First, we quickly discard T-Trees because this method shows lower predictive performance as compared to other methods, while being computationally demanding.
...
For simulations presented later, we first restrict the dataset to controls in order to remove the genetic structure induced by the celiac disease status. 
...
The C+T (Clumping + Thresholding) method directly derives a Polygenic Risk Score (PRS) from the results of a Genome-Wide Associations Study (GWAS).
In the GWAS, a coefficient of regression (i.e.\ the estimated effect size $\hat\beta_j$) is learned independently for each SNP $j$ along with a corresponding p-value $p_j$. 
...
Moreover, the penalized logistic regression consistently provides better predictive performance than the C+T method for all disease architectures tested, excepted in some simulations with 3000 causal SNPs, which are cases where all methods hardly exceed 60\% of AUC (Figures \ref{fig:main-AUC-logit} and \ref{fig:supp-AUC-logit}). 
...
However, in order to accurately predict disease status for clinical interventions, best possible predictive methods should be used. 
The penalized logistic regression we present in this paper could be used on data as large as biobank datasets because it is both very memory- and computation-efficient.
