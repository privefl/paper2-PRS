...
\abstract{
Polygenic Risk Scores (PRSs) consist in combining the information across many single-nucleotide polymorphisms (SNPs) in a score reflecting the genetic risk of developing a disease. PRSs can have a major public health impact, allowing for screening campaigns to identify high-genetic risk individuals for a given disease.  The most commonly used method to derive PRSs is called ``Clumping+Thresholding'' (C+T). It uses only univariate genome-wide association studies (GWAS) summary statistics, which makes it very suitable and fast. However, PRS rely on non-optimal heuristics to choose various numerical thresholds.

We implement a logistic regression model that is suited for large scale genomic data and which provides a joint estimation of SNP effects. The R Package bigstatsr includes efficient functions to train penalized logistic regression on large datasets.

We present a comprehensive comparative study of the C+T method, our penalized logistic regression and the T-Trees algorithm, which is a derivation of random forests. Penalized logistic regression consistently achieves higher predictive performance than T-Trees while being as fast as the C+T method. Predictive performance of the C+T method is very sensitive to the threshold of inclusion of SNPs which values depends on disease architecture. Penalized logistic regression consistently achieves superior performance than the C+T method and this improvement was more pronounced when there are few effects located in nearby genomic regions such as HLA. When analysing SNP data from a case-control study,  penalized logistic regression provides an AUC of $89\%$ while the standard C+T method provides a smaller $AUC$ of $82\%$.

Instead of computing polygenic risk scores based on GWAS univariate summary statistics, we recommend to use penalized logistic regression as implemented in the R package bigsnpr to obtain more accurate risk scores.
...
\cite{abraham2013performance} showed that penalized logistic regression and penalized SVM performed better than the standard C+T method and LMMs, for a wide-range of human diseases.
...
In this study, we particularly extend the work of \cite{abraham2013performance} [RELIRE ET PRECISER] while using a simulation framework similar but more extenstive than the one used in \cite{zhou2013polygenic}. 
...
Then, we decided to remove population structure because it can affect the predictive performance of methods [REF, MARTINS?].
...
In order to simulate phenotypes, we use the Liability Threshold Model (LTM) with a prevalence of 30\% \cite[]{falconer1965inheritance}. We vary different parameters: the number of chromosomes used (either all 22 chromosomes or only chromosome 6), the size of the training set ($\{1000, 2000, \dots, 6000\}$), the number of causal variants and their location (30, 300 or 3000 anywhere on the genome or 30 only in the HLA region), the heritability (50\% or 80\%), the distribution of effects associated with causal SNPs (Normal or Laplace). We also use two different models to compute liability scores: a ``simple'' model with linear effects only, and a ``fancy'' model which combines linear, dominant and interaction-type effects.
...
Then, thresholding (T) consists in removing SNPs with p-values larger than a threshold $t$ to be determined. 
Finally, a PRS is defined as the sum of allele counts of the remaining SNPs weighted by the corresponding effect coefficients: $$\rm{PRS}_i = \sum_{j \in S_\text{clumping}} \mathds{1}\{p_j < t\} \cdot \beta_j \cdot G_{i,j}$$ where $\beta_j$ ($p_j$) are the effect sizes (p-values) learned from the GWAS. In this study, we report scores for a clumping threshold at $r^2 > 0.2$ within regions of 500kb. 
...
Penalized logistic regression provides AUC greater than 93\% for some disease architectures, nearly reaching the maximum achievable AUC of 94\%, whereas AUC values obtained with C+T method are always lower than 90\% (Figures \ref{fig:main-AUC-logit}). This situation occurs when simulating 30 causal SNPs in a linear fashion with an heritability of 80\%.
Moreover, the penalized logistic regression consistently provides better predictive performance than the C+T method for all disease architectures tested, excepted in simulations with 3000 causal SNPs anywhere. Yet, these are cases where all methods rarely exceeds 60\% of AUC (Figures \ref{fig:main-AUC-logit} and \ref{fig:supp-AUC-logit}). 
...
In practice, a particular value of the threshold of inclusion of SNPs should be chosen for the C+T method and this choice can impact its predictive performance greatly. For example, AUC ranges from less than 60\% to 90\% in a model with only 30 causal SNPs (Figures \ref{fig:main-AUC-PRS} and \ref{fig:supp-AUC-PRS}). 
