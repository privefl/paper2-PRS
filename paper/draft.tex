...
For example, for screening high-risk individuals and for presymptomatic diagnosis of the general population, it is suggested that the AUC must be greater than respectively 75\% and 99\% \cite[]{janssens2007impact}.
...
A commonly used technique, called ``P+T'' or ``C+T'' (which stands for ``Clumping and Thresholding'') -- or genetic profiling ..or even just PRS.. -- is used to derive PRSs from results of Genome-Wide Association Studies (GWASs) \cite[]{chatterjee2013projecting,dudbridge2013power,evans2009harnessing,purcell2009common,wray2007prediction}. This technique only use summary statistics which makes it very suitable and also very fast.
Linear Mixed-Models (LMMs) are another widely-used method in fields such as plant and animal breeding or for predicting highly heritable quantitative human phenotypes such as height \cite[]{lello2017accurate,yang2010common}. Yet, these models are not designed for predicting a binary trait such as a disease status and have proven to not be optimal at such task in another comparative study \cite[]{abraham2013performance}. Moreover, these methods and their derivatives are often computationally demanding, both in terms of memory and time requirements, which makes them unlikely to be used for prediction on large cohorts \cite[]{golan2014effective,maier2015joint,speed2014multiblup,zhou2013polygenic}.
...
In this study, we particularly extend the work of \cite{abraham2013performance} by using a simulation framework similar but ..richer.. than the one used in \cite{zhou2013polygenic}. In order to make our comparison as comprehensive as possible, we compare different architectures of disease (number, size and location of causal effects and heritability) with different model for generating liability ..scores.., one with only linear effects, and one which combines linear, dominant and interaction effects. First, we quickly discard T-Trees as being the best method for predicting disease status based on genotypes.
Then, we show that penalized logistic regression consistently performs better than the C+T method whereas predictive performance of the C+T method is very sensitive to the threshold of inclusion of SNPs, depending of the architecture of disease, as shown in \cite{ware2017heterogeneity}.
...
For both models, the $y_i$ are then standardized to make sure they have a variance exactly equal to the desired heritability $h^2$ and we further add some environmental noise $\epsilon_i$ to $y_i$ where $\epsilon \sim N(0, 1 - h^2)$.


We make 3 different simulation scenarios, summarized in table \ref{tab:simus}. 
Scenario \textnumero1 uses the whole dataset (the 22 chromosomes) and a training set of size 6000. It compares all methods (see section \ref{sec:methods}) on all other combination of parameters in 100 simulations (i.e.\ 5 times more than in \cite{zhou2013polygenic}). Note that the T-Trees method is compared on only 5 simulations and an heritability of 80\%, which aims at quickly discarding this method that can require more that one hour of computation for one simulation. 
Scenario \textnumero2 consists in 100 simulations comparing the ``logit-simple'' and the C+T methods on a dataset composed of only the chromosome 6. Reducing the number of SNPs aims at increasing the ..polygenicity.. of our simulated models and at ..virtually.. increasing the sample size [REF? DUDBRIDGE?]. For this scenario, we use only the additive (``simple'') model, but continue to compare all previous variations of the other parameters.
Finally, scenario \textnumero3 reuses the whole dataset but varying the size of the training set in order to assess how the number of samples affects predictive performance of the PRS and the ``logit-simple'' methods. 100 simulations are run using 300 causal SNPs anywhere on the genome. 
...
As a second measure, we report the percentage of the partial AUC for a specificity between 90\% and 100\% \cite[]{dodd2003partial,mcclish1989analyzing}. This measure is similar to the AUC, but focuses on high specificities (i.e.\ discarding most of the healthy population as being diseased) which is often the most useful part of the ROC curve in clinical settings. 
...
The C+T (Clumping + Thresholding) method directly derives a Polygenic Risk Score (PRS) from the results of Genome-Wide Associations Studies (GWASs), named summary statistics.
...
The SNPs are first clumped (C) so that there remain only loci that are weakly correlated with each other. 
...
In results, we use estimates of maximum achievable AUC values for the simulations: 84\% (respectively 94\%) for heritabilities of 50\% (respectively 80\%) based on three estimations (see Supplementary Materials). Note that we do not report the simulation results in terms of partial AUC because partial AUC values have a Spearman correlation of 98\% with the AUC results for all methods (Figure \ref{fig:supp-AUC-corr}).
...
Using the penalized logistic regression, we report AUCs greater than 93\%, nearly reaching the maximum achievable AUC of 94\%, which is not obtained by the upper bound of the C+T method in any scenario (Figures \ref{fig:main-AUC-logit} and \ref{fig:supp-AUC-logit}). 
Our penalized regression methods consistently provides better predictive performance than the C+T method for all disease architectures tested, excepted sometimes when we simulate 3000 causals SNPs anywhere on the genome, a case where all methods rarely exceeds 65\% of AUC (Figures \ref{fig:main-AUC-logit} and \ref{fig:supp-AUC-logit}). 
...
We also report that the choice of the threshold of inclusion of SNPs in the C+T method can greatly impact its performance, for example dropping from an AUC of 90\% to less than 60\% when including all SNPs passing clumping in a model with only 30 causal SNPs, or dropping from 60\% to a random predictor (50\% of AUC) when including only GWAS-significant SNPs in a model with 3000 causal SNPs (Figures \ref{fig:main-AUC-PRS} and \ref{fig:supp-AUC-PRS}). 

\subsection{``Fancy'' methods and model}

First, in scenario \textnumero1 we also tested the T-Trees method. 
For example, compared to our penalized logistic regression, T-Trees perform worse in term predictive measures, while taking much longer to run and making more complex predictive models because it uses more SNPs in the models and has non-linear effects (Figure \ref{fig:supp-ttrees}). Even when simulating a fancy model in which there is dominant and interaction effects that decision trees based methods should handle, AUC is still worse with T-Trees as compared with the ``logit-simple'' method.

Secondly, as for the comparison between our two penalized logistic regression in scenario \textnumero1, the classic one (called ``simple'') and the one using additional features coding for recessive and dominant effects (called ``triple''), we report that the ``triple'' method is nearly as good as the ``simple'' one when there are only linear effects and can lead to significantly greater results when there are also dominant and interactions effects (Figures \ref{fig:supp-triple} and \ref{fig:supp-AUC-triple}). Yet, the ``triple'' solution takes 2-3 more times to run and 3 more disk storage.

Thirdly, for the ``fancy'' simulation model, we report the exact same results as in the previous subsection that is to say that the ``logit-simple'' give greater AUCs than the ``PRS-max'', excepted in the case of 3000 causal SNP anywhere on the genome, where no method exceed 60\% of AUC (Figure \ref{fig:supp-AUC-logit-fancy}). Moreover, the choice of the threshold inclusion of SNPs also greatly affect the performance of the C+T method (Figure \ref{fig:supp-AUC-PRS-fancy}).

\subsection{Simulations varying number of SNPs and training size}

These simulations corresponds to scenarios \textnumero2 and \textnumero3 (Table \ref{tab:simus}). First, when reproducing simulations of scenario \textnumero1 on only the chromosome 6, methods globally increase or ..stagnate.. in predictive performance (Figure \ref{fig:supp-AUC-chr6}). There is particularly a high increase in the case when simulating 3000 causal SNPs anywhere on the dataset: for example, AUC from the ``logit-simple'' can increase from less than 60\% to nearly 80\%. On the contrary, when simulating only 30 causal SNPs on the corresponding dataset, AUC of the ``PRS-max'' seems to slightly decrease. Secondly, when varying the training size, we report an increase of AUC when increasing the training size, with the AUC of the ``logit-simple'' method increasing faster than the ``PRS-max'' method (Figure \ref{fig:main-AUC-ntrain}).


\subsection{Real data results}

We also analyze the whole Celiac dataset with the ..original/real.. phenotypes. For one run [ENOUGH FOR REVIEWERS?], splitting the dataset in 12,000 indiduals in the training set and the remaining 3155 individuals in the test set, we compare the predictive performance of the ``C+T'', ``logit-simple'' and ``logit-triple'' methods, but also computation times and number of predictors used in the models (Table \ref{tab:results-celiac}, figure \ref{fig:celiac-roc} and supplementary notebook ``results-celiac'').
We ..see.. that the joint logistic regressions provide higher AUC with 88.71\% with the ``logit-simple'' and 89.26 with the ``logit-triple as compared to 82.46 with the ``C+T'' method, and much higher partial AUC (specificity 0.9-1), respectively 0.0411, 0.0440 and 0.0283, while using less predictors, respectively 2198, 4617 and 13277. Note that for the ``C+T'' method, we still report the best result among 102 p-values thresholds. In terms of computation time, we report only the GWAS computation for the ``C+T'' method and we show that the ``logit-simple'' method, while learning jointly on all SNPs at once, is faster than the fast ``C+T'' method, and the ``logit-triple'' takes twice as long as the ``logit-simple''. 
Finally, note that the scores you get with the logistic regressions are probabilities, which is not the case with the ``C+T'' method (Figure \ref{fig:supp-score-densities}).[QUE RAPPORTER SUR LES DENSITY PLOTS DU SUPMAT?]

\begin{table}[h]
\caption{Results for the real Celiac dataset.\label{tab:results-celiac}}
\vspace*{0.5em}
\centering
\begin{tabular}{|l|c|c|c|c|}
\hline
Method & AUC (\%) & pAUC & \# predictors & Execution time (s) \\ \hline
PRS-max & 82.46 & 0.0283 & 13277 & 149 \\ 
logit-simple & 88.71 & 0.0411 & 2198 & 78 \\
logit-triple & 89.26 & 0.0440 & 4617 & 161 \\ \hline
\end{tabular}
\end{table}
...
In this comparative study, we propose other methods as a replacement to the widely used C+T method. 
We show that jointly estimating the SNP effects using efficient penalized logistic regressions enable to achieve really high predictive performance, as reported in a previous study \cite[]{abraham2013performance}, which is not the case with the C+T method that uses SNP effects learned independently.
...
However, in disease prediction for clinical interventions, best possible methods should be used, even if it has to be used by large consortia only [EVEN LARGE CONSORTIA DON'T HAVE ALL DATA?]. The logistic regression we present here could be used on data as large as biobank datasets because it is very efficient in both memory and computation time (..even faster than doing a GWAS!..).

%One does not simply identify high-risk individuals (Figure \ref{fig:meme}).

As for the C+T method itself, we recall that we report the maximum AUCs of 102 thresholds, a threshold that should normally be learned on the training set only. The choice of this threshold is very important as it can greatly impact the predictive performance of the method. In the penalized logistic regression we present, we remove the choice of the regularization parameter with a procedure that we call Cross-Model Selection and Averaging (CMSA) while keeping the same predictive performance.

We also introduce a simple feature engineering technique to detect and learn dominant and recessive effects.. 

[PARLER DU PROBLEME DE POP? + future paper + possible car fast and memory efficient]
