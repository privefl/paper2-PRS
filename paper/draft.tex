...
First, we show that AUC is highly correlated with the percentage of cases in either the 10\% or 20\% largest PRSs (Figure \ref{fig:AUC-corr}). So, we report only predictive performance in terms of AUC in following results.
...
We show that jointly estimating the SNP effects using efficient penalized logistic regressions enable to achieve really good performance as reported in previous studies [REF GAD + AUTRE?], better than with the P+T method.
