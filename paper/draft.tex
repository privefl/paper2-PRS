...
First, when reproducing simulations of scenario \textnumero1 using chromosome 6 only (scenario \textnumero2), the predictive performance of all methods globally increase (Figures \ref{fig:supp-AUC-all-r2} and \ref{fig:supp-AUC-chr6-all-r2}). 
...
\caption{[CHANGE] Comparison of three different threshold used in the C+T method in scenario \textnumero1. Mean of AUC over 100 simulations. Upper (lower) panel is presenting results for effets following a Gaussian (Laplace) distribution. Error bars are representing $\pm 2 \text{SD}$ of $10^5$ non-parametric bootstrap of the mean of AUC. The blue dotted line represents the maximum achievable AUC. These results correspond to the ``simple'' model and an heritability of 50\%.}
