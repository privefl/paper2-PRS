...
Note that in this comparison we don't include any LMM method for the reasons mentioned before and don't include any SVM method because it is expected to give similar results to logistic regression \cite[]{abraham2012sparsnp}.
...
In results, we use estimates of maximum achievable AUC values for the simulations: 84\% (respectively 94\%) for heritabilities of 50\% (respectively 80\%) based on three estimations (see Supplementary Materials).
...
However, in disease prediction for clinical interventions, best possible methods should be used, even if it has to be used by large consortia only [EVEN LARGE CONSORTIA DON'T HAVE ALL DATA?].
...
\newpage
\begin{table}[htbp]
\caption{Summary of all simulations. When there is symbol `-' in a box, it means that the parameters are the same as the ones in the upper box.\label{tab:simus}}
\vspace*{0.5em}
...
\newpage
\begin{figure}[h]
\centerline{\includegraphics[width=\textwidth]{main-celiac-regpath}}
\caption{``Regularization paths'' for the C+T, ``logit-simple'' and ``logit-triple'' methods for one run on the Celiac disease, training on 12,000 individuals and projecting on the remaining 3155 individuals. For the C+T method, the dotted line represents the maximum AUC while for the logistic regressions, the dotted lines represent the value using the CMSA procedure to be free of the choice of the threshold.}
\label{fig:main-celiac-regpath}
\end{figure}
...
\caption{Scenario \textnumero1. Comparison of different predictive measures. Correlation is greater than 98\%.[ADD]}
...
\newpage
\begin{figure}[h]
\centerline{\includegraphics[width=\textwidth]{supp-celiac-roc}}
\caption{ROC Curves for the C+T, ``logit-simple'' and ``logit-triple'' methods for one run on the Celiac disease, training on 12,000 individuals and projecting on the remaining 3155 individuals.}
\label{fig:supp-celiac-roc}
\end{figure}
