...
{\color{red}
Applying our model to 350,000 individuals of the UK Biobank, we predict height with a larger correlation than with the best prediction of C+T (\textasciitilde65\% instead of \textasciitilde55\%), further demonstrating its scalibility and strong predictive power, even for highly polygenic traits.
...
Our implementation of L1-penalized linear regression runs in less than one day for 350K individuals (training set), achieving a correlation of more than 65.5\% with true height for both sexes in the remaining 24K individuals (test set). By comparison, the best C+T model achieves a correlation of 55\% for women and 56\% for men (in the test set), and the GWAS part takes one hour (for the training set). 
If using only the top 100,000 SNPs from a GWAS on the training set to fit our L1-PLR, correlation between predicted and true heights drops at 0.634 for women and 0.643 for men.
...
Trait & Method & $r$ (women / men) & \# predictors & Execution time \\
  \hline
height & PLR & 0.656 / 0.657 & 115,997 & 21h \\ 
