...
{\color{red}
Using 350,000 individuals of the UK Biobank, we predict height with 10\% more correlation than with the best prediction of C+T. Although height is a highly polygenic trait with many small effects, our implementation of penalized regression proves to be scalable and, to the best of our knowledge, provides the best genetic predictor of height to date.
...
In conclusion, our study demonstrates that penalized regression can achieve much more predictive polygenic risk scores, while being applicable to large-scale individual-level data thanks to the implementation we provide in R package bigstatsr.
}
...
{\color{red}
Yet, PRS would have to show a high discriminative power between cases and controls in order to be used in clinical settings. For screening high-risk individuals and for presymptomatic diagnosis of the general population, it is suggested that the AUC must be greater than 75\% and 99\% respectively \cite[]{janssens2007impact}. Note that these AUC figures are also dependent on the prevalence of the disease.
}
...
{\color{red}
Overall, we find that PLR achieves higher predictive performance than C+T except in highly underpowered cases (AUC values lower than 0.6), while being scalable to biobank data. %This demonstrates the feasibility and relevance of this approach for PRS computation based on large individual-level datasets that require scalable methods but offer some high predictive performance in return. 
}
...
{\color{red}
For the computation of this paper, the GWAS part uses the training set while clumping uses the test set (all individuals not included in the training set).
}
...
\caption{Main comparison of C+T and PLR in scenario \textnumero1 for model ``ADD''. Mean AUC over 100 simulations for PLR and the maximum AUC reported with ``C+T-max'' (clumping threshold at $r^2 > 0.2$). Upper (lower) panels are presenting results for effets following a Gaussian (Laplace) distribution and left (right) panels are presenting results for a heritability of 0.5 (0.8). Error bars are representing $\pm 2 \text{SD}$ of $10^5$ non-parametric bootstrap of the mean AUC. The blue dotted line represents the maximum achievable AUC.}
...
{\color{red}
Finally, on this Celiac dataset, our implementation of PLR runs 300 times as fast as SparSNP \cite[]{abraham2012sparsnp}.
}
...
We tested our implementation on 656K genotyped SNPs of the UK Biobank, keeping only caucasian individuals and removing individuals related to someone else in the data (excluding the second individual in each pair with a kinship greater than 0.08). Results are presented in table \ref{tab:results-UKBB}.

Our L1-penalized linear regression runs in less than one day for 350K individuals, achieving a correlation of more than 65.5\% with true height for both sexes. The best C+T model achieves a correlation of 55\% for women and 56\% for men, where the GWAS part takes 1 hour only. 
...
Our L1-penalized logistic regression on breast cancer runs in only 13 minutes for 150K women, achieving an AUC of 0.598. The best C+T model achieves an AUC of 0.589, where the GWAS part takes 15 hours.

\begin{table}[h]
\caption{Results for the UK Biobank. The training/test sets for height (resp. breast cancer) are 350,000/24,131 (resp. 150,000/38,628). For PLR, dfmax is a parameter that controls the maximum number of variables allowed to enter the model before stopping fitting (default is 50,000 for each model corresponding to each validation set). Third row corresponds to fitting PLR on the top 100,000 SNPs from a GWAS on the training set. For height, $r$ (correlation with true height) is reported within women / men; for breast cancer, AUC is reported.\label{tab:results-UKBB}}
\vspace*{0.5em}
\centering
\begin{tabular}{|l|l|c|c|c|}
  \hline
Trait & Method & $r$ or AUC & \# predictors & Execution time \\
  \hline
...
The computation time of our PLR implementation mainly depends on the sample size and the number of active variables. Indeed, the algorithm is composed of two steps: first, for each variable, the algorithm computes one statistic that will be used to decide if the variable enters the model or not (for each value of $\lambda$). This first step should be very fast.
Then, the algorithm iterates over a regularization path of decreasing values of $\lambda$, which progressively enables variables to enter the model (Figure  \ref{fig:CMSA}). So, in the second step, the number of variables is increasing progressively, and at some point the computation stops due to the early stopping criterion (when prediction is getting worse on the corresponding validation set, see figure \ref{fig:CMSA}).

So, for highly polygenic traits such as height and when using huge datasets such as the UK Biobank, the algorithm might iterate over more than 100,000 variables, which takes time. On the contrary, for traits like the celiac disease or breast cancer that seems to be less polygenic, a lot less variables are entering the model and the fitting is very fast (13 minutes only for 150K women of the UK Biobank for breast cancer!). Using more cases (there were only 3542 for Celiac and 6768 for breast cancer), one might achieve better predictive performance for these two diseases, and the computation time should remain within a few hours.

Regarding memory requirements, this is basically the same as for computation time. Indeed, active variables are accessed in memory thanks to memory-mapping when they are used \cite[]{prive2017efficient}. 
If there are too many variables accessed, the operating system (OS) has to make some space for new coming variables. 
There should never be any memory issue thanks to memory-mapping. Yet, at some point, if there are too many active variables, the OS would regularly swap memory between disk and RAM, which is very slow. 
...
Another solution would be to reduce the `n.abort' parameter (default value is 10) in order to stop fitting the regularization path even sooner. This would avoid fitting models with a large number of variables without changing the solution since regularization paths for very large sample sizes are very smooth (Figure \ref{fig:supp-path}).
We have quickly tested processing each chromosome separately and then combining the models later but this did not perform as well as fitting all SNPs at once (data not shown). 
Another widely used approach is to apply the penalized regression on a subset of SNPs by prioritizing SNPs using univariate tests (GWAS computed from the same dataset). Yet, this strategy was shown to reduce predictive power \cite[]{abraham2013performance,accuheight2018}, which we also confirm in this paper. Indeed, when using only the 100K most significantly associated SNPs, this reduces correlation between predicted and true heights from 0.657/0.658 to 0.634/0.643 within women/men, while decreasing computation time by 20\% only (Table \ref{tab:results-UKBB}).
...
When there are many effects of small size and a small sample size, PLR performs worse than (the best result for) C+T. This may be because PLR is too conservative in including effects, whereas it has been shown in the literature that, in such underpowered scenarios, including (nearly) all SNPs was the best strategy \cite[]{purcell2009common}.
In scenario \textnumero3, by varying sample size, we show that when increasing sample size, PLR achieves higher predictive performance than C+T at some point and that the benefits of using PLR over C+T increase with an increasing sample size (Figure \ref{fig:main-AUC-ntrain}). Moreover, in scenario \textnumero2, we decrease the search space (p: number of SNPs) in order to increase the polygenicity of the models and to virtually increase the sample size \cite[]{dudbridge2013power}. In this scenario \textnumero2, even when there are small effects and a high polygenicity (3000 causal variants out of 18,941), PLR gets a large increase in predictive performance, now consistently higher than C+T (Figure \ref{fig:supp-AUC-all-r2}).
}
...
Our approach has one major limitation: the main advantage of the C+T method is its direct applicability to summary statistics, allowing to leverage the largest GWAS results to date, even when individual cohort data cannot be merged because of practical or ethical reasons (e.g.\ consortium data including many cohorts). As of today, our implementation of penalized logistic regression does not allow for the analysis of summary data, but this represents an important future direction of our work. 
