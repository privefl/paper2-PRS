...
{\color{red}
Applying our model to 350,000 individuals of the UK Biobank, we predict height with a larger correlation than with the best prediction of C+T [TODO CHIFFRES FROM TO], further demonstrating its scalibility and strong predictive power even for highly polygenic traits.
...
This demonstrates the feasibility and relevance of using penalized regression for PRS computation when large individual-level datasets are available.

In conclusion, our study demonstrates that penalized regression can provide discriminative polygenic risk scores, while being applicable to large-scale individual-level data because of an efficient implementation available in R package bigstatsr.
}
}
...
Linear Mixed-Models (LMMs) are another widely-used method in fields such as plant and animal breeding or for predicting highly heritable quantitative human phenotypes such as height \cite[]{yang2010common}. Yet, models resulting from LMM, known e.g.\ as ``gBLUP'', are not optimal for predicting disease status based on genotypes (see table 2 of \cite{abraham2013performance}). 
...
{\color{red}
Overall, we find that PLR achieves higher predictive performance than C+T except in highly underpowered cases (AUC values lower than 0.6), while being scalable to biobank data. For example, this underpowered situation occurs when there are many causal variants (3K) to find among many typed variants (280K) and a small sample size (6K).  %This demonstrates the feasibility and relevance of this approach for PRS computation based on large individual-level datasets that require scalable methods but offer some high predictive performance in return. 
}
...
\begin{table}[htbp]
\caption{Summary of all simulations. Where there is symbol `-' in a box, it means that the parameters are the same as the ones in the upper box. [TODO: number of SNPs in dataset]\label{tab:simus}}
\vspace*{0.5em}
\centering
\begin{tabular}{|c|c|c|c|c|c|c|c|}
\hline
Numero of & \multirow{2}{*}{Dataset} & Sample size & Causal SNPs & Distribution & \multirow{2}{*}{Heritability} & Simulation & \multirow{2}{*}{Methods} \\ 
scenario & & of training set & (number and location) & of effects & & model & \\
\hline
\hline
...
the L1- and L2-regularization (``elastic-net'', \cite{zou2005regularization}) is a compromise between the two previous penalties and is particularly useful in the $p \gg n$ situation ($p$ is the number of SNPs), or any situation involving many correlated predictors (corresponds to $0 < \alpha < 1$) \cite[]{friedman2010regularization}. In this study, we use a grid search over $\alpha \in \{1,~0.5,~0.05,~0.001\}$, which is embedded in our PLR implementation.
...
This type of algorithm builds predictions for many values of $\lambda$, which is called a ``regularization path''. To obtain an algorithm that does not require to choose this hyper-parameter $\lambda$, we developed a procedure that we call Cross-Model Selection and Averaging (CMSA, figure \ref{fig:CMSA}).
...
Moreover, PLR consistently provides higher predictive performance than C+T across all scenarios we considered, except in some cases of high polygenicity or small sample size where all methods perform poorly (AUC values below 60\% -- figures \ref{fig:main-AUC-logit} and \ref{fig:main-AUC-ntrain}).
...
\caption{Main comparison of C+T and PLR when simulating phenotypes with additive effects (scenario \textnumero1, model ``ADD'') [TODO: chnages for every plot]. Mean AUC over 100 simulations for PLR and the maximum AUC reported with ``C+T-max'' (clumping threshold at $r^2 > 0.2$). Upper (lower) panels are presenting results for effets following a Gaussian (Laplace) distribution and left (right) panels are presenting results for a heritability of 0.5 (0.8). Error bars are representing $\pm 2 \text{SD}$ of $10^5$ non-parametric bootstrap of the mean AUC. The blue dotted line represents the maximum achievable AUC.}
...
Concerning the $r^2$ threshold of the clumping step in C+T, we mostly used the common value of $0.2$. Yet, using a more stringent value of $0.05$ provides higher predictive performance than using $0.2$ in most of the cases we considered (Figures \ref{fig:supp-AUC-all-r2} and \ref{fig:main-AUC-ntrain})
...
Joint logistic regressions also provide higher AUC values for the Celiac data: 88.7\% with PLR and 89.1\% with PLR3 as compared to 82.5\% with ``C+T-max'' (Table \ref{tab:results-celiac} and figure \ref{fig:celiac-roc}).
...
In terms of computation time, we show that PLR, while learning jointly on all SNPs at once and testing four different values for hyper-parameter $\alpha$, is almost as fast as the C+T method (190 vs 130 seconds), and PLR3 takes less than twice as long as PLR (296 vs 190 seconds). [TODO: reformuler SparSNP reponse]
...
Our implementation of L1-penalized linear regression runs in less than one day for 350K individuals, achieving a correlation of more than 65.5\% with true height for both sexes [TODO: IN.. ajouter test set]. By comparison, the best C+T model achieves a correlation of 55\% for women and 56\% for men, and the GWAS part takes 1 hour. 
If using only the top 100,000 SNPs from a GWAS on the training set to train our L1-PLR, correlation between predicted and true heights drops at 0.634 for women and 0.643 for men.
Our L1-penalized logistic regression on breast cancer runs in 13 minutes for 150K women, achieving an AUC of 0.598 [TODO test set]. The best C+T model achieves an AUC of 0.589, and the GWAS part takes 15 hours.

\begin{table}[h]
\caption{[TODO deplace dfmax supmat] Results for the UK Biobank. The training/test sets for height (resp. breast cancer) are 350,000/24,131 (resp. 150,000/38,628). For PLR, dfmax is a parameter that controls the maximum number of variables allowed to enter the model before stopping fitting (default is 50,000 for each model corresponding to each validation set). Third row corresponds to fitting PLR on the top 100,000 SNPs from a GWAS on the training set. For height, $r$ (correlation with true height) is reported within women / men; for breast cancer, AUC is reported.\label{tab:results-UKBB}}
\vspace*{0.5em}
\centering
\begin{tabular}{|l|l|c|c|c|}
  \hline
Trait & Method & $r$ (women / men) or AUC & \# predictors & Execution time \\
  \hline
height & PLR (dfmax = 50e3) & 0.656 / 0.657 & 115,997 & 21h \\ 
height & PLR (dfmax = Inf)  & 0.657 / 0.658 & 148,341 & 61h \\ 
height & PLR (100K top SNPs, dfmax = Inf)  & 0.634 / 0.643 & 70,162 & 48.5h \\ 
height & C+T-max & 0.549 / 0.561 & 45,570 & 69m \\ 
breast cancer & PLR & 0.598 & 2653 & 13m \\ 
breast cancer & C+T-max & 0.589 & 21 & 15h \\
  \hline
\end{tabular}
\end{table}
...
When there are many small effects and a small sample size, PLR performs worse than (the best result for) C+T. For example, this underpowered situation occurs when there are many causal variants (3K) to find among many typed variants (280K) and a small sample size (6K). 
...
{\color{red}
We did not want to choose the C+T parameters as, to our knowledge, there is no gold standard to do so. So, for C+T, we report the best AUC value on the test set, even if it leads to over-optimistic results for C+T as compared to PLR.
}
...
For highly polygenic traits such as height and when using huge datasets such as the UK Biobank, the algorithm might iterate over more than 100,000 variables, which is computationally demanding. On the contrary, for traits like celiac disease or breast cancer that are less polygenic, the number of variables included in the model is much smaller so that fitting is very fast (13 minutes only for 150K women of the UK Biobank for breast cancer). %Using more cases (there were only 3542 for Celiac and 6768 for breast cancer), one might achieve better predictive performance for these two diseases, and the computation time should remain within a few hours.

Memory requirements are tightly linked to computation time. Indeed, active variables are accessed in memory thanks to memory-mapping when they are used \cite[]{prive2017efficient}. 
If there are too many variables accessed, the operating system (OS) must free some memory for new coming variables. 
There should never be any memory issue thanks to memory-mapping. Yet, if there are too many active variables, the OS would regularly swap memory between disk and RAM, severely slowing down computations. 
%If this happens for very large datasets, you can try to use less processing cores (as active sets tend to have non-overlapping variables because of LD) or buy more RAM. 
%Another solution would be to reduce the `n.abort' parameter (default value is 10) in order to stop fitting the regularization path even sooner. 
[TODO: add doc function]
%This would avoid fitting models with a large number of variables without changing the solution since regularization paths for very large sample sizes are very smooth (Figure \ref{fig:supp-path}).
%We tested processing each chromosome separately and then combining the models later but this did not perform as well as fitting all SNPs at once (data not shown). 
...
Indeed, when using only the 100K most significantly associated SNPs, this reduces correlation between predicted and true heights from 0.657/0.658 to 0.634/0.643 within women/men, while decreasing computation time by 20\% only (Table \ref{tab:results-UKBB}). 
A key advantage of our implementation of PLR is that prior filtering of variables is no more required for computational feasibility.

}

\subsection{Limitations}

Our approach has one major limitation: the main advantage of the C+T method is its direct applicability to summary statistics, allowing to leverage the largest GWAS results to date, even when individual cohort data cannot be merged because of practical or ethical reasons (e.g.\ consortium data including many cohorts). As of today, our implementation of penalized logistic regression does not allow for the analysis of summary data, but this represents an important future direction. 
...
Supplemental Data also include six HTML R notebooks including all code and results used in this paper, for reproducibility purposes, and available at \url{https://figshare.com/articles/code/7178750}. Additional analyses of the UK Biobank are available as 3 R scripts at \url{https://figshare.com/articles/code_UKB/7531559}.
