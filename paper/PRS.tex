...
On the contrary, when simulating only 30 or 300 causal SNPs with the corresponding dataset, AUC of ``C+T-max'' does not increase, and even decreases for an heritability of 80\% (Figure \ref{fig:supp-AUC-all-r2}). 
...
If there are too many variables accessed, the operating system (OS) has to make some space for new comping variables. 
So, there should never be any memory issue thanks to memory-mapping, yet, at some point, if there are too many active variables, the OS would regularly swap memory between disk and RAM, which is very slow. 
...
We have quickly tested processing each chromosome separately and then combining the models later but this did not seem to perform as well as fitting all SNPs at once (data not shown). 
Another widely used approach is to apply the penalized regression on a subset of SNPs using univariate tests (GWAS) to prioritize SNPs. Yet, this strategy was shown to reduce predictive power \cite[]{abraham2013performance,accuheight2018}, which we also confirm here by achieving a correlation of 65.5\% for height using 350K individuals of the UK Biobank, whereas \cite{accuheight2018} achieved only 61\% for the same sample size when using only 50K SNPs (see figure 3 of their paper).
...
When there are many effects of small size and a small sample size, PLR performs worse than (the best result for) C+T. We think this is because PLR is too conservative in including effects, whereas it has been shown in the literature that in such underpowered scenarios, including (nearly) all SNPs was the best strategy \cite[]{purcell2009common}.
In scenario \textnumero3, by varying sample size, we showed that with increasing sample size, PLR would achieve higher predictive performance than C+T at some point and that the difference between the two methods would increase with an increasing sample size (Figure \ref{fig:main-AUC-ntrain}). Moreover, in scenario \textnumero2, we decrease the search space (p: number of SNPs) so that it could increase the polygenicity of the models and virtually increase the sample size (because power is dependent on n/p). In this scenario, even there are small effects and a high polygenicity (3000 causal variants out of the ~18K SNPs of chromosome 6), PLR gets a large increase in predictive performance, consistently higher than C+T (Figure \ref{fig:supp-AUC-all-r2}).
}
...
\caption{Main comparison of C+T and PLR in scenario \textnumero1 for model ``ADD''. Mean AUC over 100 simulations for PLR and the maximum AUC reported with ``C+T-max''. Upper (lower) panels are presenting results for effets following a Gaussian (Laplace) distribution and left (right) panels are presenting results for an heritability of 0.5 (0.8). Error bars are representing $\pm 2 \text{SD}$ of $10^5$ non-parametric bootstrap of the mean AUC. The blue dotted line represents the maximum achievable AUC.}
...
\caption{Comparison of methods in scenario \textnumero2 (using chromosome 6 only) for model ``ADD''. Thinner lines represents results in scenario \textnumero1. Mean AUC over 100 simulations for PLR and the maximum values of C+T for three different $r^2$ thresholds ($0.05$, $0.2$ and $0.8$) as a function of the number and location of causal SNPs. Upper (lower) panels are presenting results for effets following a Gaussian (Laplace) distribution and left (right) panels are presenting results for an heritability of 0.5 (0.8). Error bars are representing $\pm 2 \text{SD}$ of $10^5$ non-parametric bootstrap of the mean AUC. The blue dotted line represents the maximum achievable AUC.}
