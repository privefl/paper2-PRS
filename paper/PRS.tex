...
{\color{red}
Overall, we find that PLR achieves higher predictive performance than C+T except in highly underpowered cases (AUC values lower than 0.6), while being as fast. This demonstrates the feasibility and relevance of this approach for PRS computation based on large individual-level datasets that require scalable methods but offer some high predictive performance in return. 
}
...
Another widely used approach is to apply the penalized regression on a subset of SNPs using univariate tests (GWAS) to prioritize SNPs. Yet, this strategy was shown to reduce predictive power \cite[]{abraham2013performance,accuheight2018}, which we also confirm here by achieving a correlation of 65.5\% for height using 350K individuals of the UK Biobank, whereas \cite{accuheight2018} achieved only 61\% for the same sample size when using only the 50K most significantly associated SNPs (see figure 3 of their paper).
...
In scenario \textnumero3, by varying sample size, we showed that when increasing sample size, PLR would achieve higher predictive performance than C+T at some point and that the benefits of using PLR over C+T would increase with an increasing sample size (Figure \ref{fig:main-AUC-ntrain}). Moreover, in scenario \textnumero2, we decrease the search space (p: number of SNPs) in order to increase the polygenicity of the models and to virtually increase the sample size \cite[]{dudbridge2013power}. In this scenario, even when there are small effects and a high polygenicity (3000 causal variants out of 18,941), PLR gets a large increase in predictive performance, now consistently higher than C+T (Figure \ref{fig:supp-AUC-all-r2}).
}
