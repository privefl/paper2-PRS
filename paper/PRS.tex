...
In conclusion, our study demonstrates that penalized regression can achieve much more discriminative polygenic risk scores, while being applicable to large-scale individual-level data thanks to the implementation we provide in R package bigstatsr.
}
...
We tested our implementation on 656K genotyped SNPs of the UK Biobank, keeping only caucasian individuals and removing individuals related to someone else in the data (excluding the second individual in each pair with a kinship greater than 0.08). Our L1-penalized linear regression runs in less than one day for 350K individuals, achieving a correlation of 65.5\% with true height for both sexes. The best C+T model achieves a correlation of 55\% for women and 56\% for men, where the GWAS part takes 1 hour only.
...
Another widely used approach is to apply the penalized regression on a subset of SNPs using univariate tests (GWAS) to prioritize SNPs. Yet, this strategy was shown to reduce predictive power \cite[]{abraham2013performance,accuheight2018}, which we also confirm here by achieving a correlation of 65.5\% for height using 350K individuals of the UK Biobank, whereas \cite{accuheight2018} achieved only 61\% for the same sample size when using only the 50K most significantly associated SNPs [TODO: COMPARER PLUTOT A MOI DIRECTEMENT].
...
In scenario \textnumero3, by varying sample size, we show that when increasing sample size, PLR achieves higher predictive performance than C+T at some point and that the benefits of using PLR over C+T increase with an increasing sample size (Figure \ref{fig:main-AUC-ntrain}). Moreover, in scenario \textnumero2, we decrease the search space (p: number of SNPs) in order to increase the polygenicity of the models and to virtually increase the sample size \cite[]{dudbridge2013power}. In this scenario, even when there are small effects and a high polygenicity (3000 causal variants out of 18,941), PLR gets a large increase in predictive performance, now consistently higher than C+T (Figure \ref{fig:supp-AUC-all-r2}).
}
