...
methods.color <- setNames(
  c(scales::hue_pal()(6), c("black", "blue", "green")), 
  c("logit-simple", "PRS-max", "PRS-stringent", "T-Trees", "logit-triple", "PRS-all", 
    "biglasso", "logit-simple-enet-ridge", "logit-simple-lasso")
) 
```
...
```{r}
results8 <- list.files("results8", full.names = TRUE) %>%
  map_dfr(~readRDS(.x)) %>%
  format_results()
```

```{r, fig.asp=0.65, out.width="95%", fig.width=16}
compare_logit(results8, c("logit-simple-enet-ridge", "logit-simple-lasso"))

ggsave("figures/supp-alpha.pdf", scale = 1/100, width = 1600, height = 1000)
```
