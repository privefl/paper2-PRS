...
```{r}
paste(sprintf("%.1e", 10^(-c(0, -log10(5e-8), exp(seq(log(0.1), log(100), length.out = 100))))), collapse = ", ")
...
### First simulations (with T-Trees)
...
### Second simulations (without T-Trees, with logit-triple)
...
### Third simulations (without logit-triple)

```{r}
params.grid3 <- expand.grid(
  par.causal = list(c(30, "all"), c(300, "all"), c(3000, "all"), c(30, "HLA")), 
  par.dist   = c("gaussian", "laplace"), 
  par.h2     = c(0.5, 0.8), 
  par.model  = c("simple"),
...
for (i in rows_along(params.grid3)) {
  
  res.file <- paste0("results3/simu_", i, ".rds")
  if (file.exists(res.file)) next
  
  params <- params.grid3[i, ]
  par.causal <- params[["par.causal"]][[1]]
...
### Fourth simulations (dataset with only chromosome 6)
...
### Fifth simulations (varying training size)
