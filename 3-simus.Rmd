...
logit.CMSA <- function(G, pheno.all, covar.all, ind.train, ind.test,
                       method = "logit-simple", alphas = 0.5) {
...
logit.biglasso <- function(G3, pheno.all, covar.all, ind.train, ind.test) {
  ...
  timing <- system.time({
    ...
    biglasso <- biglasso(G3, pheno.all, ind.train, ncores = NCORES,
...
for (i in rows_along(params.grid7)) {
  ...
  # Get results from all methods
  res <- bind_rows(
    logit.CMSA(G, pheno.all, covar.all, ind.train, ind.test),
...
### Scenario n°1 (comparison of different $\alpha$)

```{r, eval=TRUE}
if (!dir.exists("results8")) dir.create("results8")

for (i in rows_along(params.grid7)) {
  
  res.file <- paste0("results8/simu_", i, ".rds")
  if (file.exists(res.file)) next
  ...
  res <- bind_rows(
    logit.CMSA(G, pheno.all, covar.all, ind.train, ind.test, 
               method = "logit-simple-enet-ridge", alphas = 0.001),
    logit.CMSA(G, pheno.all, covar.all, ind.train, ind.test, 
               method = "logit-simple",            alphas = 0.5),
    logit.CMSA(G, pheno.all, covar.all, ind.train, ind.test, 
               method = "logit-simple-lasso",      alphas = 1)
  )
  params[["res"]] <- list(res)
  saveRDS(unnest(params, res, .drop = FALSE), file = res.file)
}
