...
We use similar quality controls as done in \cite{qian2020fast}; 337,475 ``White British'' individuals and 504,139 genetic variants remain.
We also use the same splitting strategy: 20\% test, 20\% validation and 60\% training.
To use the same sets for bigstatsr as for snpnet, we use the same test set, use K=4 folds for training, and make sure one of them is composed of the same 20\% of the data used for validation in snpnet. 
Moreover, we use penalty factors to effectively use unscaled genotyped in bigstatsr (see next section Recommendations), as performed by default in snpnet.
...
Note that, to make the most of the training set, bigstatsr uses CMSA (Figure \ref{fig:CMSA}) while \cite{qian2020fast} propose to refit the model using the best $\lambda$ identified using one validation fold in snpnet.
...
\begin{table}[h]
	...
	\begin{tabular}{|l|c|c|c|c|c|c|}
		...
		\hline
		Asthma & 0.1354 & 0.1443 & 188 + 101 & 0.1353 & 0.1499 & 10 \\
		High cholesterol & 0.1223 & 0.1325 & 101 + 146 & 0.1226 & 0.1347 & 7 \\
		\hline
	\end{tabular}
\end{table}
...
We have shown both theoretically and empirically that bigstatsr provides faster and possibly more accurate models than snpnet, and also made some recommendations on how to best use penalized regression for deriving polygenic scores based on very large individual-level genetic data.
...
While it is true that Cox regression is not yet implemented in bigstatsr, it is one of the regression for which it is easy to derive strong rules \cite[]{tibshirani2012strong}.
...
Yet, we use a simpler and more pragmatic alternative in bigstatsr and show that this leads to faster fitting of penalized regressions in bigstatsr compared to snpnet.
