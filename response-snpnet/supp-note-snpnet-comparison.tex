...
Since our package bigstatsr is not using this BASIL procedure, one might think that fitting penalized regression with R package bigstatsr should be slow for very large datasets.
...
As we show later, this means that fitting penalized regressions using the approximation we proposed in \cite{prive2019efficient} is computationally more efficient than using the BASIL procedure proposed by \cite{qian2020fast}, and yet provides as accurate predictors.
